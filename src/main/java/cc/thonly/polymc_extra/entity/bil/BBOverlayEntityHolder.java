...
public abstract class BBOverlayEntityHolder<E extends Entity, A extends AnimatedEntity> extends AbstractAnimationHolder implements AnimatedEntityHolder {
    ...
    protected BBOverlayEntityHolder(E entity, A animatedEntity, Model model) {
        super(model);
        this.additionalDisplays = new ObjectOpenHashSet<>();
        this.entity = entity;
        this.animatedEntity = animatedEntity;

        this.dimensions = entity.getType().getDimensions();
        this.tickCount = entity.tickCount - 1;
    }
    ...
    public boolean addAdditionalDisplay(DisplayElement element) {
        if (this.additionalDisplays.add(element)) {
            ...
            this.sendPacket(new ClientboundSetPassengersPacket(this.entity));
            return true;
        }
        return false;
    }
    ...
    public void sendDirectPassengers(Consumer<Packet<ClientGamePacketListener>> consumer) {
        ...
        if (!passengers.isEmpty()) {
            consumer.accept(VirtualEntityUtils.createClientboundSetPassengersPacket(this.entity.getId(), passengers));
        }
    }
    ...
    protected void onDataLoaded() {
        this.onDimensionsUpdated(this.entity.getDimensions(this.entity.getPose()));
        super.onDataLoaded();
    }

    @Override
    protected boolean shouldSkipTick() {
        int parentTickCount = this.entity.tickCount;
